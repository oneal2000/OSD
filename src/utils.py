...
def predict_qa_llm(model, tokenizer, generation_config, question, with_cot):
    model.eval()
    input_ids = get_prompt_llm(
        tokenizer, 
        question, 
        with_cot = with_cot)
    input_len = len(input_ids)
...
def predict_fc(model, tokenizer, generation_config, input, passages):
    model.eval()
    input_ids = get_prompt_fc(
        tokenizer, 
        input, 
        passages = passages)
    input_len = len(input_ids)
...
def predict_fc_llm(model, tokenizer, generation_config, input):
    model.eval()
    input_ids = get_prompt_fc_llm(
        tokenizer, 
        input)
    input_len = len(input_ids)
    input_text = tokenizer.decode(input_ids, skip_special_tokens=True)
...
def predict_sf(model, tokenizer, generation_config, input, template_question, passages):
    model.eval()
    input_ids = get_prompt_sf(
        tokenizer, 
        input, 
        template_question,
        passages = passages)
...
def predict_sf_llm(model, tokenizer, generation_config, input, template_question):
    model.eval()
    input_ids = get_prompt_sf_llm(
        tokenizer, 
        input, 
        template_question)
    input_len = len(input_ids)
