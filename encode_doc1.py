...
# # this function is adopted from zhihu
# def save_pretrained_mixed(
#         self,
#         save_directory: str,
...
def save_pretrained_mixed(
    ...
    **kwargs: Any,
    ...
    """
    轻量化版 save_pretrained：
    只保存指定 adapter 参数 + adapter_config，不会 dump 整个模型 state_dict。
    保存结果仍兼容 PeftModel.from_pretrained 和 model.load_adapter。
    """
    if os.path.isfile(save_directory):
        raise ValueError(f"save_pretrained received a file path ({save_directory}), should be a directory.")
    ...
    for adapter_name in selected_adapters:
        ...
        # 保存目录：default 直接放根目录，其它 adapter 用子目录
        adapter_save_dir = save_directory if adapter_name == "default" else os.path.join(save_directory, adapter_name)
        os.makedirs(adapter_save_dir, exist_ok=True)

        # 保存参数
        if safe_serialization:
            from safetensors.torch import save_file as safe_save_file
            safe_save_file(adapter_state_dict, os.path.join(adapter_save_dir, "adapter_model.safetensors"))
        else:
            torch.save(adapter_state_dict, os.path.join(adapter_save_dir, "adapter_model.bin"))
...
def train(model, data, tokenizer, args, save_path, task_lora_path, doc_lora_init_path):
    ...
    model = PeftMixedModel.from_pretrained(model, task_lora_path, adapter_name="0", is_trainable=False)

    model.load_adapter(doc_lora_init_path, adapter_name="1", is_trainable=True)

    model.set_adapter(["0", "1"])

    for name, param in model.named_parameters():
        if "lora_A.0" in name or "lora_B.0" in name:
            param.requires_grad = False
        elif "lora_A.1" in name or "lora_B.1" in name:
    ...
    for epoch in range(args.num_train_epochs):
        ...
        # for step, batch in enumerate(train_dataloader):
            ...
            # pre_params = {}
            # for name, param in model.named_parameters():
            #     if any(x in name for x in [".lora_A.0", ".lora_B.0", ".lora_A.1", ".lora_B.1"]):
            #         pre_params[name] = param.detach().clone()
            
            outputs = model(**batch)
            ce_loss = outputs.loss
            ortho_loss = orthogonal_loss(model, doc_adapter_name="1", task_adapter_name="0")
            # print("ortho_loss.requires_grad:", ortho_loss.requires_grad)
            # print("ortho_loss.grad_fn:", ortho_loss.grad_fn)
    ...
    # save_pretrained_mixed(model, save_path, selected_adapters=["1"])
